...
<input type="hidden" name="view" value="reservations">
        <?php
			$pacientId = intval($_SESSION["user_id"]);
			$pacients = PacientData::getByPacient($pacientId);
...
<option value="">PACIENTE</option>
  <?php foreach($pacients as $p):?>
    <option value="<?php echo $p->id; ?>" <?php if(isset($_SESSION["user_id"]) && $_SESSION["user_id"]==$p->id){ echo "selected"; } ?>><?php echo $p->id." - ".$p->name." ".$p->lastname; ?></option>
  <?php endforeach; ?>
</select>
		</div>
    </div>
...
if($_GET["q"]!=""||$_GET["pacient_id"]!="" ||$_GET["medic_id"]!="" ){
	...
	$sql .= " date_at = \"".$_GET["date_at"]."\"";
		...
		$pacientId = intval($_SESSION["user_id"]);
		$users = ReservationData::getAllByPacientId($pacientId);
