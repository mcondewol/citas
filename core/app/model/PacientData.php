...
class PacientData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (name,lastname,dpi,gender,day_of_birth,address,phone,email,password,created_at) ";
		$sql .= "values (\"$this->name\",\"$this->lastname\",\"$this->dpi\",\"$this->gender\",\"$this->day_of_birth\",\"$this->address\",\"$this->phone\",\"$this->email\",\"$this->password\",$this->created_at)";
		Executor::doit($sql);
	} 


	public function checkPacient($email){
...
// partiendo de que ya tenemos creado un objecto PacientData previamente utilizamos el contexto
	...
	public static function getByPacient($id){
		$sql = "select * from ".self::$tablename." where id=$id";
		$query = Executor::doit($sql);
		return Model::many($query[0],new PacientData());
	}
	...
	public static function getPacientEmail($email){
